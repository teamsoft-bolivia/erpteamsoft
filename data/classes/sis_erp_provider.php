...
require_once '../include.php';
        ...
        public function __call($option,$att) {
            ...
            switch ($option)
            {
                ...
                case 'readProveedores':
                    ...
                    $sql ="SELECT * FROM erp_provider WHERE identerprise=$this->_identerprise ORDER BY idprovider ASC";
                ...
                case 'readcboproveedores':
                    $select="idprovider,providername";
                    if (!isset($datos['query'])) {
                        $datos['query']='';
                    }
                    if ( $datos['idprovider']>0 && $datos['query']=='') {
                        $idprovider=$datos['idprovider'];
                        $conditions =array("identerprise=? AND idprovider=?",  $this->_identerprise,$idprovider);
                    }else if (isset($datos['query']) && $datos['query']!='') {
                        $conditions =array("identerprise=? AND providername ilike '%".$datos['query']."%'",  $this->_identerprise);
                    }else{
                        $conditions =array("identerprise=?",-100);
                    }
                    
                    
                    try {
                        $providers= erp_provider::find('all',array('select'=>$select,
                                'conditions'=>$conditions));
                        $total=0;//count($providers);
                        $json='';
                        foreach ($providers as $datas) {
                        $json.= $datas->to_json().',';
                        }
                        $json=substr($json, 0,  strlen($json)-1);
                ...
                break;
                case 'updateCreateProvider':
                    
                    $data=array();
                    //echo 'entro';
                    //$data['idprovider']=$datos['idprovider'];
                    
                    if($datos['idprovider']==0){
                        if(isset ($datos['active'])){
                            $bool='true';
                        }else{
                            $bool='false';
                        };
                        $data['providername']=$datos['providername'];
                        $data['address']=$datos['address'];
                        ...
                        $data['idcountry']=12;
                        //print_r($data);
                       $result=erp_provider::create($data);
                            if ($result) {
                        ...
                        $data['active']=$bool;                    
                        $options['idprovider']=(int)$datos['idprovider'];

                        $result=erp_provider::update_all(array('set'=>$data,'conditions'=>$options));
                        if ($result) {
                            echo '{"success":true,"title":"Correcto:","msg":"Se guardo correctamente!!"}';
                ...
                case 'deleteprovider':
                    ...
                    if($total>0){
                        echo '{"success":false,"title":"Advertencia:","msg":"Existe asignado este proveedor!!"}';
                           ...
                           $post = erp_provider::find($iditemlistprice);
    ...
    $provider=new sis_erp_provider($iduser,$identerprise);
