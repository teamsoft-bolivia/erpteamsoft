...
require_once '../include.php';
    class sis_erp_employee_language 
    {
        public $_iduser;
        public $_tablename;
        ...
        public function __construct($usuario,$enterprise) {
            ...
            $this->_tablename= erp_employee_language::table_name();
        ...
        public function __call($option,$att) {
            ...
            switch ($option)
            {
                    ...
                    switch ($yaction) {
                        case 'insert':
                            ...
                            foreach ($datos as $key => $value) {
                                if ($key!='xaction' && $key!='yaction' && $key!='idlanguage' && $key!='null') {
                                    $data[$key]=$value;
                                }
                            }
                            try {
                                $language=  new erp_employee_language($data);
                                $language->save();
                        ...
                        case 'update':
                            ...
                            $idlanguage=$datos['idlanguage'];
                            $data=array();
                            
                            foreach ($datos as $key => $value) {
                                if ($key!='xaction' && $key!='yaction' && $key!='idlanguage' && $key!='idemployee' && $key!='null') {
                                    $data[$key]=$value;
                                }
                            }
                            try {
                                $language=erp_employee_language::find($idlanguage);
                ...
                case 'read':
                    $idemployee=$datos['idemployee'];
                        $select='erp_employee_language.*,read.type as readname,write.type as writename,speaking.type as speakingname';
                        $conditions=array("erp_employee_language.idemployee=?",$idemployee);
                        ...
                        $order='erp_employee_language.idlanguage DESC';
                        try {
                            $language=  erp_employee_language::find('all',array('select'=>$select,'joins'=>$join,'conditions'=>$conditions,'order'=>$order));
                        } catch (Exception $e) {
                            print_r($e);
                        }
    ...
    $language=new sis_erp_employee_language($iduser,$identerprise);
    $language->$xaction($_POST);
?>
