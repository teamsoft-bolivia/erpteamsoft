...
Ext.define('Erp.controller.almacenes.transacciones.ControllerTxnTransferencias', {
    extend: 'Ext.app.Controller',
    stores: ['almacenes.transacciones.StoreTxnStore',
             'almacenes.transacciones.StoreDTxnStore',
             'crm.StoreCrmEmployee',
             'StoreType'],
    views: ['almacenes.transacciones.transferencias.WindowTxnTransferencias',
    ...
    refs: [
        {
            ref     :   'WindowTxnTransferencias',
            selector:   'windowtxntransferencias'
        },
        {
            ref     :   'GridTxnTransferencias',
            selector:   'gridtxntransferencias'
        }
    ...
    init: function(e) {
        ...
        this.control({
              ...
              'windowtxntransferencias buttongroup button':{
                    click:this.actionButton
              },
              'windowtxntransferencias button[option=documents]':{
                    click:function(button){
                        var window=button.up('windowtxntransferencias');
                        var formtxn=window.down('formtxntransferencias');
              ...
              'windowtxntransferencias button[option=back]':{
                    click:function(button){
                        var window=button.up('windowtxntransferencias');
                        var formtxn=window.down('formtxntransferencias');
                        var gridtxn=window.down('gridtxntransferencias');
              ...
              'formtxntransferencias':{
                  deactivate:function(form){

                  }
              },
              'gridtxntransferencias':{
                    beforeedit:function(editor,e,eOpts){
                        var win=e.grid.up('window');
                        var formtxn=win.down('formtxntransferencias');
              ...
              'gridtxntransferencias button[option=addItem]':{
                  click:function(button){
                      var me=this;
                      //console.log('jejeje');
                      var window=button.up('window');
                      
                        var form123=window.down('formtxntransferencias');
                        var idformcab123=form123.down('numberfield[option=cabeceraid]');
                        
                        //if(){this.saveTxn(window);}
                        var grid=button.up('gridtxntransferencias');
                        //var count=griddoc.getStore().getCount();
                        console.log(grid);
              ...
              'formtxntransferencias combobox[option=comboorigen]':{
                  select:function(cbo,records,eOps){
                    var win=cbo.up('window');
                    var grid=win.down('gridtxntransferencias');
                    grid.idorigen=records[0].data.idstore;
                    console.log(grid);
                }
              },
              'gridtxntransferencias actioncolumn':{
    ...
    initAll:function(window,eOpts){
        //console.log('ggg');
        var me=this;
        var record=window.down('formtxntransferencias').getRecord();
        ...
        var griddetail=window.down('gridtxntransferencias');
    ...
    actionButton:function(button){
        var me=this;
        var window=button.up('windowtxntransferencias');
        var form=window.down('formtxntransferencias');
        switch (button.option) {
            ...
            case 'btnEditar':
                if (form.down('combobox[name=state]').getValue()==43) {
                   window.stateMode="E";
                    ...
                    me.showMsg({title:'Alerta:',msg:'Solo puede editar transferencias en REVISI&Oacute;N!!'},false);
                }
                    ...
                    Ext.MessageBox.confirm('Alerta:', 'Esta seguro Aprobar este documento??.', 
                            function(option){
                                if (option=='yes') {
                                    ...
                                    //me.saveTxn(window);
                                   // var grilla=window.down('gridtxntransferencias').getStore().load();
                                    me.terminarTransferencia(window,form);
                                    //window.down('gridtxntransferencias').getStore().load();
                                    //window.listatransfer.getStore().load();
            ...
            case 'btnAnular':
                if (window.stateMode=="R" && form.down('combobox[name=state]').getValue()==86) {
        ...
        if (window.stateMode=="N") {
            ...
            if (formtxn.getForm().isValid()) {
                ...
                if (window.stateMode=='N' && idformcab.getValue()==null) {
                    yaction='insertCabeceraTransferencia';
                }else if (window.stateMode=='E' || idformcab.getValue()!=null) {
                    yaction='updateCabeceraTransferencia';
                }
    ...
    enableControls:function(window){
        var formHead=window.down('formtxntransferencias');
        console.log(window.stateMode);
    ...
    enableForm:function(window,form,stateMode){
        var me=this;
        var griddet=window.down('gridtxntransferencias');
        //var griddoc=window.down('griddoccompras');
        ...
        if (stateMode=='R') {
           Ext.each(items,function(item,index,allitems){
                ...
                if(me.permitirElegir==false){
                           Ext.each(items,function(item,index,allitems){
                                if (item.name!='idtxnstore' && item.name!='correlative' && item.name!='state' && item.name!='originstore' && item.name!='destinationstore') {
                                    item.setReadOnly(0);
                                }
    ...
    guardardetalle:function(editor, e,eopts) {
      ...
      actColumnDetail:function(gridview,el,rowindex,colindex,e,record){
        ...
        var grid=window.down('gridtxntransferencias');
        var formtxn=window.down('formtxntransferencias');
        var idcombo=formtxn.down('combobox[option=comboorigen]');
        var idcombodest=formtxn.down('combobox[option=combodestino]');
        //console.log(window.stateMode);
